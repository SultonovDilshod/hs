...
window.Icon = ({name, size=18, className=''}) => {
  const icons = {
    ...
    trendUp: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg>,
    trendDown: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 17 13.5 8.5 8.5 13.5 2 7"/><polyline points="16 17 22 17 22 11"/></svg>,
    ...
    'share-2': <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>,
    ...
    maximize: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M8 3H5a2 2 0 00-2 2v3m18 0V5a2 2 0 00-2-2h-3m0 18h3a2 2 0 002-2v-3M3 16v3a2 2 0 002 2h3"/></svg>,
  };
...
window.StatusBadge = ({status}) => {
  const styles = {
    ...
    archived: 'bg-txt-muted/15 text-txt-muted border-txt-muted/30',
    new: 'bg-accent-cyan/15 text-accent-cyan border-accent-cyan/30',
    strengthened: 'bg-status-green/15 text-status-green border-status-green/30',
    weakened: 'bg-status-amber/15 text-status-amber border-status-amber/30',
  };
  const labels = { active:'Faol', draft:'Qoralama', archived:'Arxiv', new:'Yangi', strengthened:'Kuchaygan', weakened:'Susaygan' };
  return <span className={`tag border ${styles[status]}`}>{labels[status]}</span>;
};
    ...
    <div className="glass rounded-lg px-3 py-2 text-xs" style={{border:'1px solid #E2E8F0'}}>
      ...
      {payload.map((p,i) => (
        <p key={i} style={{color:p.color}} className="flex items-center gap-2">
          ...
          {p.name}: <span className="font-semibold text-txt-primary">{typeof p.value === 'number' && p.value > 999 ? formatNumber(p.value) : p.value}{p.name?.includes('rate') || p.name?.includes('Rate') ? '%' : ''}</span>
        </p>
      ))}
    </div>
  );
